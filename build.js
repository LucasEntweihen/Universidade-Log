...
function report(rel, before, after, copied) {
  ...
  if (!copied && before > 0) {
    ...
    console.log(`  ${rel.padEnd(48)} ${before.toString().padStart(7)} -> ${after.toString().padStart(7)} bytes  (-${pct}%)`);
  }
}
...
(async () => {
  ...
  const savedPct = totals.before ? (100 * savedBytes / totals.before).toFixed(1) : '0';
  ...
  console.log(`Economia:     ${(savedBytes / 1024).toFixed(1)} KB (${savedPct}%)`);
