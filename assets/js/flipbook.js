...
(function() {
    ...
    pages.forEach((p, idx) => {
        if (p.showHeader && !(p.letter in letterPageIndex))
            letterPageIndex[p.letter] = idx;
    });
    ...
    function render() {
        ...
        leftEl.querySelector(".fb-page-content").innerHTML = renderPageHTML(
            leftLeaf,
            "left",
        );
        rightEl.querySelector(".fb-page-content").innerHTML = renderPageHTML(
            rightLeaf,
            "right",
        );
        leftEl.querySelector(".fb-page-number").textContent =
            pageNumberLabel(current);
        rightEl.querySelector(".fb-page-number").textContent = pageNumberLabel(
            current + 1,
        );

        document.getElementById("fbPrevBtn").disabled = current <= 0;
        document.getElementById("fbNextBtn").disabled =
            current + 2 >= leaves.length;
        document.getElementById("fbProgress").textContent =
            `folha ${Math.floor(current / 2) + 1} de ${Math.ceil(leaves.length / 2)}`;
    ...
    function doSearch(q) {
        ...
        if (hit) {
            const L = hit.term
                ...
                .normalize("NFD")
                .replace(/[\u0300-\u036f]/g, "");
            if (document.getElementById("fbCover").style.display !== "none")
                openBook();
            jumpToLetter(L);
        }
    }
    ...
    document.addEventListener("keydown", (e) => {
        if (!document.querySelector(".fb-book-wrap").classList.contains("open"))
            return;
