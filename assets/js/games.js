      ...
      setTimeout(() => {
        ...
        if (state.matched === state.cards.length / 2) {
          ...
          if (typeof toast === "function")
            toast(
              '<img class="om om-sm" src="../assets/icons/content/confetti.svg" alt=""> Jogo completo!',
              "Todos os pares encontrados em " +
                state.attempts +
                " tentativas.",
            );
          if (typeof mascotBounce === "function")
            mascotBounce("Boa! Memorizou tudo certinho.");
        }
      }, 500);
    } else {
...
function clickSeqChip(id, chipEl, origIdx) {
  ...
  if (origIdx === state.next) {
    ...
    if (state.next === state.total) {
      ...
      if (typeof toast === "function")
        toast(
          '<img class="om om-sm" src="../assets/icons/content/confetti.svg" alt=""> Sequência perfeita!',
          "Você ordenou tudo certinho.",
        );
      if (typeof mascotBounce === "function")
        mascotBounce("Ordem perfeita! Isso é entender o processo.");
    }
  } else {
    chipEl.classList.remove("wrong");
...
function finishDlgGame(id) {
  ...
  if (typeof toast === "function")
    toast(
      '<img class="om om-sm" src="../assets/icons/content/confetti.svg" alt=""> Diálogo concluído!',
      state.correctCount + " de " + total + " certas.",
    );
  if (typeof mascotBounce === "function")
    mascotBounce("Adorei ver vocês debatendo isso.");
...
function renderScrambleWord(id) {
  ...
  if (statusEl)
    statusEl.innerText =
      "Palavra " + (state.idx + 1) + "/" + state.words.length;
  target.innerHTML = "";
...
function clickScrambleLetter(id, btnEl, ch) {
  ...
  if (ch === nextChar) {
    ...
    const slot = root.querySelector(
      '.scr-slot[data-pos="' + (filled.length - 1) + '"]',
    );
    if (slot) {
      slot.innerText = ch;
    ...
    if (filled.length === word.length) {
      ...
      if (statusEl)
        statusEl.innerText =
          "Resolvida! " + (state.idx + 1) + "/" + state.words.length;
      setTimeout(() => {
        if (state.idx < state.words.length - 1) {
          ...
          if (typeof toast === "function")
            toast(
              '<img class="om om-sm" src="../assets/icons/content/confetti.svg" alt=""> Todas decifradas!',
              state.words.length + " palavras desembaralhadas.",
            );
          if (typeof mascotBounce === "function")
            mascotBounce("Vocabulário afiado! Isso é dominar os termos.");
        }
      }, 550);
    }
  } else {
...
function renderTFRound(id) {
  ...
  root.querySelector(".tf-status").innerText =
    state.idx +
    1 +
    "/" +
...
function finishSpeedTF(id) {
  ...
  root.querySelector(".tf-statement").innerText =
    state.correct + "/" + total + " certas nessa rodada relâmpago.";
  root.querySelector(".tf-status").innerText =
    state.correct === total
      ? "Rodada perfeita!"
      : 'Toque em "jogar de novo" pra tentar bater esse placar.';
  ...
  if (state.correct === total) {
    ...
    if (typeof mascotBounce === "function")
      mascotBounce("Reflexo total! Nenhuma pegadinha passou.");
  } else if (typeof mascotTip === "function") mascotTip(true);
  if (typeof toast === "function")
    toast(
      '<img class="om om-sm" src="../assets/icons/content/confetti.svg" alt=""> Relâmpago concluído!',
      state.correct + " de " + total + " certas.",
    );
}
...
function renderTQRound(id) {
  ...
  root.querySelector(".tq-status").innerText =
    state.idx +
    1 +
    "/" +
...
function resolveTQ(id, chosenIdx) {
  ...
  if (chosenIdx >= 0 && !correct && opts[chosenIdx])
...
function finishTQ(id) {
  ...
  root.querySelector(".tq-question").innerText =
    state.correct + "/" + total + " certas contra o relógio.";
  ...
  if (state.correct === total) {
    if (typeof confettiBurst === "function") confettiBurst(26);
    if (typeof mascotBounce === "function")
      mascotBounce("Cronômetro derrotado! Pontuação perfeita.");
  }
  if (typeof toast === "function")
    toast(
      '<img class="om om-sm" src="../assets/icons/content/confetti.svg" alt=""> Contra o Relógio concluído!',
      state.correct + " de " + total + " certas.",
    );
}
...
document.addEventListener("DOMContentLoaded", () => {
  document
    .querySelectorAll(".memory-game")
    .forEach((el) => initMemoryGame(el.id));
  document
    .querySelectorAll(".sequence-game")
    .forEach((el) => initSequenceGame(el.id));
  document
    .querySelectorAll(".dialogue-game")
    .forEach((el) => initDialogueGame(el.id));
  document
    .querySelectorAll(".scramble-game")
    .forEach((el) => initScrambleGame(el.id));
  document
    .querySelectorAll(".speedtf-game")
    .forEach((el) => initSpeedTFGame(el.id));
  document
    .querySelectorAll(".timedquiz-game")
    .forEach((el) => initTimedQuizGame(el.id));
});
