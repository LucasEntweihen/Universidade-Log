...
(function () {
  ...
  const LEVEL_STEP = 90;
  const LEVEL_TITLES = C.levelTitles || [
    "Iniciante",
    "Aprendiz",
    "Praticante",
    "Competente",
  ...
  const NODE_ORDER =
    C.nodeOrder || Array.from({ length: TOTAL_MODULES }, (_, i) => i + 1);
  const MASCOT_TIPS = C.mascotTips || [
    "Sem pressa - cada tarefa marcada já é progresso.",
  ];
  const ACH_MODULES =
    C.achModules ||
    NODE_ORDER.map((m, i) => ({
      ...
      id: "module" + m + "_master",
      name: "Módulo " + (i + 1),
      icon: `<img class="om om-sm" src="${ICO}checkered-flag.svg" alt="">`,
    }));
  ...
  const ACHIEVEMENTS = [
    ...
    {
      id: "quiz_perfect",
      cond: () => Object.keys(quizState).length >= TOTAL_QUIZZES,
    },
    { id: "streak_3", cond: () => getStreak().count >= 3 },
    ...FIB_ACH.map((f) => ({
      id: f.id,
      cond: () => getStreak().count >= f.days,
    })),
    { id: "focus_1", cond: () => focusSessions >= 1 },
    {
      id: "dialogue_master",
      cond: () => localStorage.getItem(PREFIX + "_dialogue_v1") === "done",
    },
  ...
  const ACH_NAMES = {
    ...
    streak_3: "Maratonista",
    focus_1: "Foco Total",
    dialogue_master: C.dialogueAchName || "Bate-papo Concluído",
    completionist: C.completionistName || "Mestre do Curso",
  };
  const ACH_ICON = {
  ...
  ACH_MODULES.forEach((a) => {
    ACH_NAMES[a.id] = a.name;
    ACH_ICON[a.id] = a.icon;
  });
  FIB_ACH.forEach((f) => {
    ACH_NAMES[f.id] = f.name;
    ACH_ICON[f.id] = f.icon;
  });
  ...
  function saveProgress() {
    const data = {};
    document.querySelectorAll(".c-item input[type=checkbox]").forEach((chk) => {
  ...
  function anyChecked() {
    return Array.from(
      document.querySelectorAll(".c-item input[type=checkbox]"),
    ).some((c) => c.checked);
  }
  function allChecked() {
  ...
  function anyCheckedForModule(m) {
    return Array.from(
      document.querySelectorAll('.c-item input[data-mod="' + m + '"]'),
    ).some((c) => c.checked);
  }
  ...
  function resetProgress() {
    if (
      !confirm(
        "Isso vai zerar checklist, XP, conquistas, vidas e sessões salvas - sua plantinha volta a ser semente. Confirma?",
      )
    )
      return;
    [CHECK_KEY, QUIZ_KEY, ACH_KEY, FOCUS_KEY, LEVEL_KEY, HEARTS_KEY].forEach(
      (k) => localStorage.removeItem(k),
    );
    document
      .querySelectorAll(".c-item input[type=checkbox]")
      .forEach((c) => (c.checked = false));
    quizState = {};
    unlockedAch = {};
    focusSessions = 0;
    lastKnownLevel = 1;
    document.querySelectorAll(".quiz-fb").forEach((f) => {
      f.className = "quiz-fb";
      f.innerText = "";
    });
    document
    ...
    document
      .querySelectorAll(".badge-tile")
      .forEach((b) => b.classList.remove("unlocked"));
    loadHearts();
    renderHearts();
    updateProgress();
    updateGamification();
  ...
  function loadHearts() {
    ...
    let h;
    try {
      h = JSON.parse(localStorage.getItem(HEARTS_KEY));
    } catch (e) {
      h = null;
    }
    if (!h || h.last !== today) {
  ...
  function saveHearts() {
    localStorage.setItem(
      HEARTS_KEY,
      JSON.stringify({ count: hearts, last: new Date().toDateString() }),
    );
  }
  const HEART_SVG =
    '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M16.5 3C19.5376 3 22 5.5 22 9C22 16 14.5 20 12 21.5C9.5 20 2 16 2 9C2 5.5 4.5 3 7.5 3C9.35997 3 11 4 12 5C13 4 14.64 3 16.5 3Z"></path></svg>';
  ...
  function loseHeart() {
    if (hearts > 0) {
      ...
      if (hearts === 0) {
        mascotShake(
          "Sem vidas por hoje - mas você pode continuar estudando sem pressa. Elas voltam amanhã.",
        );
        toast(
          '<img class="om om-sm" src="' +
            ICO +
            'broken-heart.svg" alt=""> Vidas esgotadas',
          "Sem problema - continue estudando à vontade, sem perder mais vidas hoje.",
        );
      } else {
      ...
      mascotShake(
        "Tudo bem, sem mais vidas hoje não tem problema nenhum em continuar aprendendo.",
      );
    }
  }
  ...
  function updateStreakDisplay() {
    ...
    if (el)
      el.innerText =
        s.count +
        (s.count === 1 ? " dia de estudo seguido" : " dias de estudo seguidos");
  }
    ...
    focusInterval = setInterval(() => {
      ...
      if (focusRemaining <= 0) {
        ...
        toast(
          '<img class="om om-sm" src="' +
            ICO +
            'stopwatch.svg" alt=""> Sessão concluída!',
          "+20 XP de bônus por foco",
        );
  ...
  function computeXP() {
    const checked = document.querySelectorAll(
      ".c-item input[type=checkbox]:checked",
    ).length;
  ...
  function titleForLevel(lv) {
    ...
    return lv > LEVEL_TITLES.length
      ? base + " ★" + (lv - LEVEL_TITLES.length)
      : base;
  }
  ...
  function updateGamification() {
    ...
    if (lb) lb.innerText = lv;
    const lt = document.getElementById("levelTitle");
    if (lt) lt.innerText = titleForLevel(lv);
    const lx = document.getElementById("levelXp");
    if (lx) lx.innerText = xpIntoLevel + " / " + LEVEL_STEP + " XP";
    const xf = document.getElementById("xpFill");
    if (xf)
      xf.style.width = Math.min(100, (xpIntoLevel / LEVEL_STEP) * 100) + "%";

    const stageEl = document.getElementById("mascotStageEmoji");
    if (stageEl)
      stageEl.innerHTML =
        GROWTH_STAGES[Math.min(lv - 1, GROWTH_STAGES.length - 1)];
  ...
  function showLevelUp(lv) {
    ...
    if (num) num.innerText = "Nv. " + lv;
    const ti = document.getElementById("levelupTitle");
    if (ti) ti.innerText = titleForLevel(lv);
    ...
    ACHIEVEMENTS.forEach((a) => {
      if (!unlockedAch[a.id] && a.cond()) {
        ...
        const tile = document.querySelector(
          '.badge-tile[data-badge="' + a.id + '"]',
        );
        if (tile) tile.classList.add("unlocked");
        if (!silent) {
          toast(
            ACH_ICON[a.id] + " Conquista desbloqueada!",
            ACH_NAMES[a.id],
            "badge",
          );
          confettiBurst(28);
  ...
  function toast(title, body, cls) {
    ...
    el.innerHTML =
      '<div class="t-title"><img class="om om-sm" src="../assets/icons/speaker.svg" alt="">' +
      title +
  ...
  function lightningAnsweredIds() {
    try {
      return JSON.parse(localStorage.getItem(LIGHTNING_KEY) || "[]");
    } catch (e) {
      return [];
    }
  }
  function markLightningAnswered(id) {
    ...
    if (!done.includes(id)) {
      ...
      localStorage.setItem(LIGHTNING_KEY, JSON.stringify(done));
    }
  }
  function boltSvg() {
  ...
  function buildLightningCard(item, uid) {
    ...
    const optsHtml = item.options
      .map(
        (opt, i) =>
          `<button class="lightning-opt" onclick="answerLightning('${uid}',${i})">${opt}</button>`,
      )
      .join("");
    card.innerHTML = `
  ...
  function answerLightning(uid, idx) {
    ...
    fb.innerText = correct
      ? "Na mosca! +XP relâmpago."
      : item.explain || "Quase! Vale revisar esse ponto.";
    markLightningAnswered(item.id);
    if (correct) {
      if (typeof confettiBurst === "function") confettiBurst(16);
      if (typeof toast === "function")
        toast(
          boltSvg() + " Desafio Relâmpago!",
          "Resposta certa na hora.",
          "badge",
        );
      if (typeof mascotBounce === "function")
        mascotBounce("Reflexo bom! Continua assim.");
    }
  }
    ...
    sections.forEach((sec) => {
      ...
      for (let n = 0; n < perSection && qi < pool.length; n++) {
        let idx,
          attempts = 0;
        do {
          idx = 1 + Math.floor(Math.random() * (blocks.length - 1));
          attempts++;
  ...
  function setupReveal() {
    const targets = document.querySelectorAll(
      ".card,.checklist,.quiz,table,.flip-grid,.project-card,.badges-grid,.steps,.game-card",
    );
    const io = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
  ...
  window.addEventListener("DOMContentLoaded", function () {
    ...
    setInterval(() => {
      ...
      if (b && !b.classList.contains("show")) mascotTip();
    }, 22000);
  });
})();
