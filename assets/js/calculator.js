...
(function () {
      ...
      if (tk.t === "id") {
        ...
        switch (name) {
          ...
          case "cos":
            return Math.cos(rad);
          case "tan":
            return Math.cos(rad) === 0 ? NaN : Math.tan(rad);
          case "asin":
            return calcDegMode
  ...
  window.calcSolveTriangle = function () {
    ...
    if (isNaN(v1) || isNaN(v2)) {
      out.innerHTML =
        '<p class="quiz-fb no" style="display:block;">Preencha os dois valores pedidos pra esse modo.</p>';
      return;
    }
    ...
    try {
      ...
      out.innerHTML =
        ...
        "<p><strong>Ângulo α:</strong> " +
        fmt(angle) +
        "°<br>" +
        "<strong>Cateto oposto:</strong> " +
        fmt(oposto) +
        "<br>" +
      ...
      out.innerHTML =
        '<p class="quiz-fb no" style="display:block;">Não foi possível resolver com esses valores.</p>';
    }
  };
  ...
  window.calcGoldenRatio = function () {
    ...
    if (isNaN(v) || v <= 0) {
      out.innerHTML =
        '<p class="quiz-fb no" style="display:block;">Digite um valor positivo.</p>';
      return;
    }
    ...
    out.innerHTML =
      ...
      "<p><strong>Segmento total:</strong> " +
      fmt(total) +
      "<br>" +
      "<strong>Parte maior:</strong> " +
      fmt(maior) +
      "<br>" +
      "<strong>Parte menor:</strong> " +
      fmt(menor) +
      "<br>" +
      "<strong>Conferência (maior/menor):</strong> " +
      fmt(maior / menor) +
      "</p></div>";
  };
  ...
  window.switchCalcTab = function (name) {
    ...
    document.querySelectorAll(".calc-body").forEach((p) => {
