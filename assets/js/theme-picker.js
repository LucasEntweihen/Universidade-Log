...
(function () {
  ...
  var SWATCH_COLORS = {
    light: '#EDEAE2', dark: '#2B2620',
    blue: '#DCEBFA', 'blue-dark': '#1B2A3D',
    green: '#DEF2E1', 'green-dark': '#1B2B20',
    purple: '#E7E1FA', 'purple-dark': '#241E38',
    amber: '#FBE8CE', 'amber-dark': '#2E2013',
    rose: '#FBE2EA', 'rose-dark': '#301621',
    space: '#7B6CFF'
  };
  ...
  window.setTheme = function (theme) {
    ...
    document.querySelectorAll('.theme-swatch,.pf-swatch').forEach(function (el) {
      var isActive = el.dataset.theme === theme;
      el.classList.toggle('active', isActive);
      if (el.tagName === 'BUTTON') el.setAttribute('aria-pressed', isActive ? 'true' : 'false');
    });
    ...
    document.querySelectorAll('.theme-popover-panel').forEach(function (panel) {
      panel.closest('.theme-popover') && panel.closest('.theme-popover').classList.remove('open');
    });
  };
  ...
  window.toggleThemePopover = function (btn) {
    ...
    document.querySelectorAll('.theme-popover.open').forEach(function (w) { w.classList.remove('open'); });
  ...
  document.addEventListener('DOMContentLoaded', function () {
    var current = 'light';
    try { current = localStorage.getItem(STORAGE_KEY) || 'light'; } catch (e) {}
    document.querySelectorAll('.theme-swatch,.pf-swatch').forEach(function (el) {
