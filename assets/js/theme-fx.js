...
(function () {
  ...
  var COURSES = ['course-ia', 'course-android', 'course-devops', 'course-trigomante'];
  var course = null;
  for (var i = 0; i < COURSES.length; i++) {
    if (body.classList.contains(COURSES[i])) { course = COURSES[i]; break; }
  }
    ...
    function update() {
      ...
      for (var i = 0; i < wraps.length; i++) {
        ...
        if (course === 'course-android') {
          img.style.transform = 'translateX(' + (progress * 70).toFixed(1) + 'px)';
        } else {
          img.style.transform = 'translateY(' + (progress * 90).toFixed(1) + 'px)';
        }
      }
      ticking = false;
    }
  ...
  function setupStarfield() {
    ...
    var configs = [
      { count: 26, size: [1, 2], speed: 0.035, dur: [3, 6] },
      { count: 14, size: [2, 3.4], speed: 0.075, dur: [2.4, 4.5] }
    ];
    var groups = configs.map(function (cfg) {
      ...
      for (var i = 0; i < cfg.count; i++) {
        ...
        var size = (cfg.size[0] + Math.random() * (cfg.size[1] - cfg.size[0])).toFixed(1);
        s.style.width = size + 'px';
        s.style.height = size + 'px';
        s.style.left = (Math.random() * 100).toFixed(1) + '%';
        s.style.top = (Math.random() * 100).toFixed(1) + '%';
        s.style.setProperty('--fx-dur', (cfg.dur[0] + Math.random() * (cfg.dur[1] - cfg.dur[0])).toFixed(1) + 's');
        s.style.setProperty('--fx-delay', (Math.random() * 4).toFixed(1) + 's');
        g.appendChild(s);
      }
      layer.appendChild(g);
      return { el: g, speed: cfg.speed };
    });
    body.prepend(layer);
    ...
    function update() {
      ...
      groups.forEach(function (g) {
        g.el.style.transform = 'translateY(' + (-(y * g.speed)).toFixed(1) + 'px)';
      });
      ticking = false;
    }
  ...
  function setupWaves() {
    ...
    var defs = [
      { color: 'var(--sea,#1B4F72)', opacity: .09, duration: '42s', height: '16vh', bottom: '0' },
      { color: 'var(--sea-2,#2E86C1)', opacity: .14, duration: '27s', height: '11vh', bottom: '0' },
      { color: 'var(--sand,#E9C46A)', opacity: .16, duration: '18s', height: '6vh', bottom: '0' }
    ];
  ...
  function setupDungeon() {
    ...
    var torchSpots = [[6, 18], [94, 30], [10, 72], [90, 60]];
